...
void context_init(int argc, char **argv)
{
	...
	while ((c = getopt(argc, argv, PING_ARG_FLAGS)) != -1)
		switch (c)
		{
			case PING_Q:
			case PING_N:
			...
			case PING_I:
				ping_ctx.flags[c] = true;
				ping_ctx.echo_interval = atoi(optarg);
				if (!(PING_INTERVAL_MIN <= ping_ctx.echo_interval && ping_ctx.echo_interval <= PING_INTERVAL_MAX))
				{
					fprintf(stderr, "%s: invalid argument: '%s': out of range: %d <= value <= %d\n",
			...
			case PING_S:
				ping_ctx.flags[c] = true;
				ping_ctx.sizeof_payload = atoi(optarg);
				if (!(PING_MIN_PAYLOAD_SZ <= ping_ctx.sizeof_payload && ping_ctx.sizeof_payload <= PING_MAX_PAYLOAD_SZ))
				{
					fprintf(stderr, "%s: invalid argument: '%s': out of range: %d <= value <= %d\n",
	...
	err_code = get_ip_address(
			ping_ctx.dest,
			&ping_ctx.dest_address,
			ping_ctx.canon_dest,
...
static void set_default_args()
{
	...
	ping_ctx.echo_interval = 1;

	ping_ctx.count_messages_sent = 0;
	ping_ctx.count_error_messages_received = 0;
	ping_ctx.count_messages_received = 0;
	ping_ctx.min_time = UINT64_MAX;

	if (gettimeofday(&ping_ctx.start_time, NULL) != 0)
	{
